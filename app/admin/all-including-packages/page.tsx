...
export default function AllIncludingPackagesPage() {
  ...
  const [data, setData] = useState(
    (allIncludes || []).map(pkg => ({ ...pkg, id: pkg.includeId }))
  );
  ...
  useEffect(() => {
    setTotalPackages((allIncludes || []).length);
  }, [allIncludes]);
  ...
  const applyFilters = () => {
    ...
    if (filters.search) {
      filteredData = filteredData.filter(pkg =>
        pkg.includeId.toLowerCase().includes(filters.search.toLowerCase()) ||
  ...
  const handleCreatePackage = () => {
    router.push('/admin/all-including-packages/create');
  };

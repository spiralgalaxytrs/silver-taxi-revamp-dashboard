...
export default function OutstationOneWayPrice() {
    ...
    useEffect(() => {
        fetchServices();
        const filtered = services.find(service => service.name === "One way");
    ...
    const handleSave = async (entry: PriceEntry) => {
        try {
            ...
            console.log("satee - ", status, message)
            if (status === 201 || status === 200) {
                toast.success(editingEntry ? "One way Price updated successfully" : "One way created successfully", {
                    style: {
        ...
        <>
            <div className="p-6 space-y-6 bg-white rounded-lg shadow">
                ...
                <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                    {priceEntry &&
                        <PriceCard key={priceEntry.priceId} entry={priceEntry as PriceEntry} onEdit={handleEdit} onDelete={handleDelete} serviceName="One Way" />
                    }
                </div>
            </div>
        </>

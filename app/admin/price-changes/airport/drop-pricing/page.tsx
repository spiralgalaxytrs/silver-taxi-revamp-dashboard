...
export default function AirportDropPrice() {
  const { fetchEntriesById, CreateEntry, updateEntry, deleteEntry, isLoading, error } = usePriceStore()
  ...
  useEffect(() => {
    fetchServices();
    const filtered = services.find(service => service.name === "Airport Drop");
  ...
  const handleSave = async (entry: PriceEntry) => {
    try {
      ...
      if (status === 201 || status === 200) {
        toast.success(editingEntry ? "Airport Drop Price updated successfully" : "Airport Drop created successfully", {
          style: {
  ...
  const priceEntry = usePriceStore.getState().priceEntry
    ...
    <>
      <div className="p-6 space-y-6 bg-white rounded-lg shadow">
        <div className="rounded bg-white p-5 shadow">
          <h1 className="text-3xl font-bold tracking-tight">Airport Drop Pricing</h1>
        </div>
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {priceEntry &&
            <PriceCard key={priceEntry.priceId} entry={priceEntry as PriceEntry} onEdit={handleEdit} onDelete={handleDelete} serviceName="Airport Drop" />
          }
        </div>
      </div>
    </>
